{% macro form(form, form_class='', title_class='', button_class='button-link', title='Оставьте заявку и получите консультацию', button='Оставить заявку', form_text='', add_fields, exclude_fields) %}
...
<div class="form__item form__item_{{ type }} {{ 'd-none' if type == 'hidden' }}">
    ...
    {% else %}
    <input name="{{ name }}" type="{{ type }}" placeholder="{{ placeholder }}" class="form__input" {{ 'required' if required }} value="{{ value }}">
    {% endif %}
</div><!-- form__item -->
{% endmacro %}